...
export default function Auth() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`UI Test: ${isLogin ? 'Logged in' : 'Signed up'}!`);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden text-[var(--text-main)] p-4 md:p-8">
      ...
      <motion.div 
        ...
        transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1] }} // Custom springy easing
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              type="submit"
              className="w-full py-3.5 mt-4 bg-brand-600 hover:bg-brand-500 text-white font-semibold rounded-xl shadow-[0_0_15px_rgba(249,115,22,0.3)] hover:shadow-[0_0_25px_rgba(249,115,22,0.5)] transition-all duration-300 flex items-center justify-center gap-2"
            >
              {isLogin ? 'Access Dashboard' : 'Create Account'} <ArrowRight className="w-4 h-4" />
            </motion.button>
          </form>
